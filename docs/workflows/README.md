...
| Phase | 문서 | 기간 | Task 수 | 완료 | 범위 |
|-------|------|------|---------|------|------|
| Phase 1 | [phase-1.md](phase-1.md) | Week 1~4 | 21개 (T-101 ~ T-121) | 7/21 | 기반 구축: DB, 엔티티, JWT, 강의 CRUD, React UI |
| Phase 2 | [phase-2.md](phase-2.md) | Week 5~8 | 14개 (T-201 ~ T-214) | 0/14 | 핵심 학습: 진도 추적, 퀴즈/과제, 온보딩, 커뮤니티 |
| Phase 3 | [phase-3.md](phase-3.md) | Week 9~12 | 20개 (T-301 ~ T-320) | 0/20 | 이벤트 인프라 + AI 기반: Outbox, AI Gateway, PII, OTel, FinOps |
| Phase 4 | [phase-4.md](phase-4.md) | Week 13~16 | 24개 (T-401 ~ T-424) | 0/24 | RAG + AI 튜터: Chunking, Hybrid Search, SSE, AI 채점 |
| Phase 5 | [phase-5.md](phase-5.md) | Week 17~19 | 15개 (T-501 ~ T-515) | 0/15 | 분석 + 품질 관리: 학습 분석, RAGAS, DeepEval, A/B 테스트 |
| Phase 6 | [phase-6.md](phase-6.md) | Week 20~24 | 22개 (T-601 ~ T-622) | 0/22 | 고도화 및 완성: Flutter, 알림, Grafana, Chaos Test, 배포 |
...
> **⚠ 에이전트는 코드 작성 전에 반드시 해당 Phase workflow 문서를 읽어야 한다.**

1. **착수 전**: `docs/workflows/phase-{N}.md`에서 대상 Task의 서브 스텝·완료 기준·규칙 체크리스트 확인
2. **착수 시**: `TASKS.md`에서 상태를 `IN_PROGRESS`로 변경
3. **진행 중**: 서브 스텝 완료 시 → 해당 워크플로우의 체크박스 체크 (`- [ ]` → `- [x]`)
4. **완료 시**: 모든 완료 기준 충족 → `TASKS.md`에서 상태를 `DONE`으로 변경
5. **README.md 갱신**: Phase별 완료 카운트 업데이트
...
| v4.1 | 2026-04-08 | Phase 1 Week 1 (T-101~T-107) 완료 반영, 완료 카운트 추가, workflow 확인 필수 절차 추가 |
